...
-- deliverable 1   Retrieve employees by most recent job title about to retire
SELECT COUNT(title),
title
...
-- Deliverable 3A  Show number of retiring titles through end of 1960 to illustrate continuation of silver tsunami
SELECT  -- columns we want in new table
    e.emp_no, 
    e.first_name,
    e.last_name,
    ...
    t.from_date,
    t.to_date
INTO retirement2_titles  -- name of new table
FROM employees as e  -- 1st table to join with alias
INNER JOIN titles as t  -- 2nd table to join with alias and join type
ON (e.emp_no = t.emp_no)  -- column that is equal between the 2 tables
WHERE (e.birth_date BETWEEN '1956-01-01' AND '1960-12-31')  -- only those born between these dates
ORDER BY e.emp_no;  --setting the order in ascending
-- create unique table for titles in the following 5 years. 
SELECT DISTINCT ON (emp_no) emp_no,
first_name,
last_name,
title
INTO unique2_titles
FROM retirement2_titles
ORDER BY emp_no, (to_date = '9999-01-01') DESC;
--  counting titles reaching retirement age in the following 5 years. 
SELECT COUNT(title),
title
...
-- Deliverable 3B  Create MORE menotorship eligibility table
SELECT DISTINCT ON (e.emp_no)  -- selecting the columns we want
    e.emp_no,
    e.first_name,
    e.last_name,
...
LEFT JOIN titles as ti  -- left join because the from and to_date columns are different between titles and dept_emp which causes errors. and alias
on (e.emp_no = ti.emp_no)  -- colum that is equal
WHERE (e.birth_date  BETWEEN '1960-01-01' AND '1965-12-31')  -- limiting to birth years
	AND (de.to_date = '9999-01-01') -- limits to current employee date code
ORDER BY emp_no;  -- sets order ascending by employee number
